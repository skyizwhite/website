...
- **Two cache layers, Next.js style.** `src/lib/cache.lisp` plays the role of `next/cache`. Fetchers are defined with `deffetcher`, which is `defcached` plus a list of tags, and `revalidate-tag` clears every fetcher carrying a tag. Pages have a version each, and `revalidate-path` bumps the version of one path.
- **ETag revalidation.** `*etag-middleware*` (`src/lib/etag.lisp`) gives each public page a weak ETag, `W/"<build-id>.<page-version>"`. The build id is captured at process start (so a deploy invalidates every page); the page version comes from `revalidate-path`. Because the ETag is known before rendering, a matching `If-None-Match` is answered with `304` without running the page at all. This relies on page HTML being identical for every visitor: per-visitor state (the like button) must stay in the `/actions` fragments, which are `no-store` and bypass the middleware, as do `/assets`, `/api`, and any request with a query string.
- **On-demand revalidation.** A microCMS webhook hits `POST /api/revalidate` (auth via `X-MICROCMS-WEBHOOK-KEY`). The handler maps each webhook to the tags and paths it affects, and that mapping is the programmer's responsibility: a page that starts showing new data must be added there or it will stay stale. Tags are revalidated first and paths second, so a render that races the webhook can only tag stale data with an old version. Draft saves revalidate tags only, since draft pages carry a query string and are never tagged. No CDN purge API is involved; invalidation is driven entirely by response headers.
...
Cloudflare does not cache HTML by default, so the zone needs a Cache Rule that makes HTML eligible for cache with the edge TTL taken from the origin `Cache-Control` header. Do not set an explicit edge TTL or add `s-maxage`; Cloudflare disables `stale-while-revalidate` for both. `cf-cache-status` on a page should read `HIT`, `REVALIDATED`, `STALE` or `UPDATING` once the rule is in place. Features that rewrite the response body (Email Address Obfuscation, Automatic HTTPS Rewrites, Rocket Loader, Cloudflare Fonts, Polish, JavaScript Detections, RUM injection) must stay off: Cloudflare removes the origin `ETag` whenever one of them is on, and the `304` path silently stops working.
